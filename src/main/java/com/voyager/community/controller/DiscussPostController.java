...
import com.voyager.community.entity.*;
...
public class DiscussPostController implements CommunityConstant {
    ...
    public String addDiscussPost(@NotEmpty(message = "文章标题不能为空") String title,String content){
        ...
        if(user == null){
            return CommunityUtil.getJSONString(403,"您还未登录");//403 代表没有权限
        }
    ...
    public String getDiscussPost(@PathVariable("discussPostId") int discussPostId, Model model, Page page){
        ...
        if(commentList != null){
            for(Comment comment:commentList){
                ...
                likeCount = likeService.findEntityLikeCount(ENTITY_TYPE_COMMENT,comment.getId());//该评论的点赞数量
