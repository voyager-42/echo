...
public class LoginController implements CommunityConstant {
     ...
     * @param password 密码
     * @param code 验证码
     * @param rememberMe 是否记住我（点击记住我后，凭证有效期延长）
     * @param response
     * @param kaptchaOwner 从cookie中取出 kaptchaOwner
     * @return
     */
    @PostMapping("/login")
    public String login(@RequestParam("username") String username,
        ...
        //凭证过期时间（是否记住我）记住（100天） 默认（12小时）
        int expieredSeconds = rememberMe ? REMEMBER_EXPIRED_SECONDS:DEFAULT_EXPIRED_SECONDS;
        //验证用户名和密码
        Map<String,Object> map = userService.login(username,password,expieredSeconds);
        if(map.containsKey("ticket")){
            // 账号和密码均正确，则服务端会生成 ticket，浏览器通过 cookie 存储 ticket
            Cookie cookie = new Cookie("ticket",map.get("ticket").toString());
            cookie.setPath(contextPath);//cookie 有效范围
            cookie.setMaxAge(expieredSeconds);
            response.addCookie(cookie);
    ...
    public Map<String,Object> sendEmailCodeForResetPwd(Model model,
        ...
        //检查图片验证码
        String kaptchaCheckRst = checkKaptchaCode(kaptchaOwner,kaptcha);
        if(StringUtils.isBlank(kaptchaCheckRst)){
            map.put("status",1);
            map.put("errMsg",kaptchaCheckRst);
        }
        ...
        if(StringUtils.isBlank(usernameMsg)){
            map.put("status",0);
            map.put("msg","已经往您的邮箱发送了一份验证邮件，请注意查收!");
        }
        return map;
    }
