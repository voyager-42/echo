...
import com.voyager.community.entity.LoginTicket;
...
public class UserService implements CommunityConstant {
    ...
    public Map<String,Object> register(User user){
        ...
        //注册用户
        user.setSalt(CommunityUtil.generateUUID().substring(0,5));//salt(4个随机字符)
        ...
        user.setCreateTime(new Date()); //用户注册时间

        userMapper.insertUser(user);//将注册用户插入到数据库中
    ...
    public int activation(int userId,String code){
        ...
        if(user.getStatus() == 1){
            ...
            clearCache(userId);//用户信息变更时清除对应缓存数据
